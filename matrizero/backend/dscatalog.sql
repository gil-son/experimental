...
create table tb_network (id  bigserial not null, date TIMESTAMP WITHOUT TIME ZONE, description TEXT, img_url varchar(255), name varchar(255), price float8, primary key (id));
create table tb_network_category (network_id int8 not null, category_id int8 not null, primary key (network_id, category_id));
...
alter table tb_network_category add constraint FK5r4sbavb4nkd9xpl0f095qs2a foreign key (category_id) references tb_category;
alter table tb_network_category add constraint FKgbof0jclmaf8wn2alsoexxq3u foreign key (network_id) references tb_network;
